...
@dataclass
class SalesCollectionsByIndustry(ETLPipeline):
    """
    Fiscal year sales collections by industry.
    ...
    def __post_init__(self):
        ...
        self.path = DATA_DIR / "raw" / "by-sector" / "sales" / f"FY{fy_tag}.pdf"
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Set the columns
        data.columns = ["industry", "number_entities", "total", "percent_of_total"]
    ...
    def validate(self, data):
        ...
        main_industries = data.query(
            "parent_industry.isnull() and industry != 'Subtotal' and industry != 'Motor Vehicle Sales Tax'"
        )
        subtotal1 = main_industries["total"].sum()
        subtotal2 = data.query("industry == 'Subtotal'")["total"].squeeze()
        diff = subtotal1 - subtotal2
        assert diff < 5

        # Sub industries
        subsectors = data.query("parent_industry.notnull()")
        totals = subsectors.groupby("parent_industry")["total"].sum()

        # Compare to total
        for industry in totals.index:
            total1 = totals.loc[industry]
            total2 = data.loc[data["industry"] == industry]["total"].squeeze()
            diff = total1 - total2
            assert diff < 5
    ...
    def load(self, data) -> None:
        ...
        fy_tag = str(self.fiscal_year)[-2:]
        path = DATA_DIR / "processed" / "by-sector" / "sales" / f"FY{fy_tag}.csv"
